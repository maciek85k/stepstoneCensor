...
# macOS: Try to use MPS (Apple Silicon GPU) if available, fallback to CPU
device = -1  # Default: CPU

# Check if MPS is available (Apple Silicon GPU)
if torch.backends.mps.is_available():
    try:
        print('Apple Silicon (MPS) detected! Attempting to use GPU...')
        device = 'mps'
    except Exception as e:
        print(f'MPS not working, falling back to CPU: {e}')
        device = -1
else:
